...
import { WalrusBlobViewer } from "@/components/walrus/walrus-blob-viewer";
...
function OrderBook() {
	...
	return (
		<div className="rounded-lg border bg-card p-4 shadow-sm">
			...
			{/* 收合內容區塊 */}
			<div
				className={`transition-[max-height,opacity] duration-200 ease-in-out ${isOpen ? "max-h-[600px] opacity-100" : "max-h-0 opacity-0 overflow-hidden"
					}`}
			>
				{/* tabs */}
				<div className="flex items-center justify-between mb-3">
					<div className="inline-flex items-center rounded-full border bg-muted/40 p-0.5 text-xs">
						<button
							type="button"
							onClick={() => setSide("YES")}
							className={`px-3 py-1 rounded-full transition ${side === "YES"
						...
						<button
							type="button"
							onClick={() => setSide("NO")}
							className={`px-3 py-1 rounded-full transition ${side === "NO"
								? "bg-primary text-primary-foreground shadow-sm"
								: "text-muted-foreground hover:text-foreground"
								}`}
						>
...
function MarketContent() {
	const [oracleBlobId, setOracleBlobId] = React.useState<string | null>(null);
	...
	const { marketList, queryMarkets, fetchUser, queryOracleSettings, user } = storeStore();
	...
	React.useEffect(() => {
		if (!market?.oracle_config) return;

		const fetchBlob = async () => {
			const blob_id = await queryOracleSettings(market.oracle_config);
			console.log("blob_id:", blob_id);
			if (blob_id) {
				setOracleBlobId(blob_id);
			}
		};

		fetchBlob();
	}, [queryOracleSettings, market?.oracle_config]);
		...
		<div className="max-w-6xl mx-auto px-4 py-8">
			...
			<div className="mt-8 grid gap-8 md:grid-cols-[minmax(0,2fr)_minmax(0,1.4fr)] items-start">
				...
				<div className="space-y-6">
					...
					{oracleBlobId && (
						<div className="mt-4">
							<WalrusBlobViewer blobId={oracleBlobId} network="testnet" />
						</div>
					)}
							...
							<>
								...
								<div className="flex justify-center mb-2">
									<div className="inline-flex items-center rounded-full border bg-muted/40 p-1 text-xs font-medium">
										<button
											type="button"
											className={`px-3 py-1 rounded-full transition ${orderType === "MARKET"
												? "bg-primary text-primary-foreground shadow-sm"
												: "text-muted-foreground hover:text-foreground"
												}`}
										...
										<button
											type="button"
											className={`px-3 py-1 rounded-full transition ${orderType === "LIMIT"
												? "bg-primary text-primary-foreground shadow-sm"
												: "text-muted-foreground hover:text-foreground"
												}`}
